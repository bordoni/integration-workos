/**
 * Logout Redirect URLs – dynamic add/remove rows.
 *
 * @package WorkOS
 */
...
( function () {
	const config  = window.workosLogoutRedirectUrls || {};
	const wpRoles = config.wpRoles || {};
	const env     = config.env || '';

	const table  = document.getElementById( 'workos-logout-redirect-urls-table' );
	const tbody  = table ? table.querySelector( 'tbody' ) : null;
	const addBtn = document.getElementById( 'workos-logout-redirect-urls-add' );
	...
	function buildRoleSelect( index ) {
		...
		select.name  = 'workos_' + env + '[logout_redirect_urls][keys][' + index + ']';
	...
	function updateRemoveButtons() {
		const rows    = tbody.querySelectorAll( '.workos-logout-redirect-url-row' );
		const buttons = tbody.querySelectorAll( '.workos-logout-redirect-url-remove' );
	...
	function addRemoveToRow( row ) {
		...
		td.className = 'workos-logout-redirect-url-actions';
	...
	existingRows.forEach(
		function ( row ) {
			row.classList.add( 'workos-logout-redirect-url-row' );
			addRemoveToRow( row );
		}
	);
	...
	addBtn.addEventListener(
		...
		function () {
			...
			tdAction.className = 'workos-logout-redirect-url-actions';
	...
	tbody.addEventListener(
		'click',
		function ( e ) {
			const btn = e.target.closest( '.workos-logout-redirect-url-remove' );
			if ( ! btn ) {
				return;
			}

			const row = btn.closest( '.workos-logout-redirect-url-row' );
			if ( row ) {
				row.remove();
				updateRemoveButtons();
			}
