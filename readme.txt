...
* Fix: Username generation no longer exhausts memory when a popular email local part (e.g. `info@`) has a deep `info_1`, `info_2`, … collision chain — a failure that prevented those WorkOS users from ever getting a WordPress account. Collisions are now resolved with a short suffix derived from `sha256( email )` (e.g. `info_48f25`): at most 2 lookups regardless of chain depth, and the same email always derives the same username, so retries and replayed webhooks converge. Existing accounts keep their current usernames. ([CONS-513](https://linear.app/nexcess/issue/CONS-513)) (#27)
